...
function StateForm({ onSubmit, onCancel, initialData }) {
  ...
  // REVIEW: Optional chaining stops at `initialData?.sound` but `.type` is accessed without `?.`. If `initialData` exists but `initialData.sound` is undefined, this will throw a TypeError. Use `initialData?.sound?.type` for all sound property accesses below.
  const [soundType, setSoundType] = useState(
    initialData?.sound.type || "drone",
  );
  const [frequency, setFrequency] = useState(
    initialData?.sound.frequency || 80,
  );
  const [volume, setVolume] = useState(initialData?.sound.volume || 0.3);
  const [filterFrequency, setFilterFrequency] = useState(
    initialData?.sound.filterFreq || 800,
  );
  const [lfoSpeed, setLfoSpeed] = useState(initialData?.sound.lfoSpeed || 0.5);
  // REVIEW: previewState is a new object on every render. Since useAudioEngine's useEffect depends on [activeState] by reference, the effect re-fires every render — stopping and restarting audio continuously, causing clicks/pops. Memoize with useMemo or stabilize the reference.
  const previewState = {
    id: "preview",
    name: name,
    color: color,
  ...
  const handleSubmit = () => {
    // basic validation
    // REVIEW: Empty name is silently rejected — the user gets no feedback about why the submit didn't work. Show a validation message or disable the submit button when name is empty.
    if (!name.trim()) {
      return;
    }
    onSubmit({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={initialData ? "Edit sound state" : "Add new sound state"}
    >
      <div className={styles.form}>
        ...
        <div className={styles.actions}>
          <button
            ...
            onClick={onCancel}
            aria-label="Cancel editing sound state"
          >
            Cancel
          </button>
          <button
