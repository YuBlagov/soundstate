...
// REVIEW: `isPlaying` prop is derived from `!!analyser?.current && !isPaused` in the parent but could be simplified — the parent already knows isPaused, so isPlaying is redundant with !isPaused when audio is loaded.
function ActiveState({ activeStates, isPlaying, onPause, isPaused }) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      onKeyDown={(e) => {
        // REVIEW: Missing e.preventDefault() — pressing Space will scroll the page in addition to toggling pause.
    ...
    >
      ...
      <div className={styles.inner}>
        ...
        <span className={styles.type}>
          {isPaused ? "⏸" : primaryState.sound.type}
        </span>
      </div>
    </div>
  );
