...
// REVIEW: This hook creates an AudioContext that is never closed on unmount. If the component using this hook mounts/unmounts repeatedly (e.g., StateForm preview), AudioContexts leak. Add a cleanup effect that calls audioCtxRef.current?.close().
function useAudioEngine(activeState) {
  ...
  useEffect(() => {
    ...
    // REVIEW: No default case — if an unknown sound type is passed (e.g., from user-created states), nothing plays and the user gets no feedback. Add a default case with a console.warn.
    ...
    return () => stopAll();
    // REVIEW: The dependency array only lists [activeState], but stopAll, getAudioContext, and all play* functions are defined in the hook body and captured by this effect. Since activeState is compared by reference, passing a new object literal on every render (as StateForm does) will re-trigger this effect every render, causing audio to restart continuously.
  }, [activeState]);
